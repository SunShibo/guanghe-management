...
<div class="indexRight1">
  <div class="title">广和商学院 > 政策解读列表</div>
...
<script>
  ...
  function getdata(cur,size){
    ...
    $.getJSON("/PolicyInterpreTation/list?pageNo="+cur+"&pageSize="+size+"&title="+title, function (rs) {
  ...
  function deletePrivateInvestment(id){
    $.ajax({
      type : "post",
      url : "/PolicyInterpreTation/delete",
      data : {"newsId":id},
      dataType : "json",
      async : false,
      success : function (data){
        if(data.success == false){
          ...
          alert("删除成功！");
          window.location.href="/PolicyInterpreTation/page";
        }
      }
    });
  }
  ...
  function add(){
    window.location.href = "/PolicyInterpreTation/toAdd";
  }

  function toUpdatePage(id){
    window.location.href = "/PolicyInterpreTation/findOne?id="+id;
  }
