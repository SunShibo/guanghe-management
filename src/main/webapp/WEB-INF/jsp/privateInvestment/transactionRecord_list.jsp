    ...
    <div class="indexRight1">
        ...
        <div>
            <table class="table table-bordered">
                ...
                <thead>
                <tr>
                    <th style="width: 30%;">所管理私募基金名称</th>
                    <th style="width: 25%;">投资方向</th>
                    <th style="width: 15%;">收益情况</th>
                    <th style="width: 15%;">基金规模</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">新增历史产品及兑付情况</h4>
                </div>
                <div class="modal-body" style="margin-left:10px;">
                    <p>所管理私募基金名称：<input id="name"/></p>
                    <p>投资方向：<input id="investmentDirection"/></p>
                    <p>收益情况：<input id="profit"/></p>
                    <p>基金规模：<input id="scale"/></p>
                </div>
    ...
    <div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-top: 300px;">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title" id="">修改历史产品及兑付情况</h4>
                </div>
                <div class="modal-body" style="margin-left:10px;">
                    <p>所管理私募基金名称：<input id="name1"/></p>
                    <p>投资方向：<input id="investmentDirection1"/></p>
                    <p>收益情况：<input id="profit1"/></p>
                    <p>基金规模：<input id="scale1"/></p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
...
<script>
                ...
                if(data.success == false){
                    ...
                    for(var i=0;i<data.data.data.length;i++){
                        html += '<tr>'+
                                    ...
                                    '<td>'+ data.data.data[i].investmentDirection +'</td>'+
                                    '<td>'+ data.data.data[i].profit +'</td>'+
                                    '<td>'+ data.data.data[i].scale +'</td>'+
                                    '<td>'+
                                        '<button type="button" data-toggle="modal" data-target="#updateModal" class="btn btn-info" onclick="updateById(' + "'" + data.data.data[i].id + "',"+
                                "'" + data.data.data[i].name + "'," + "'" + data.data.data[i].investmentDirection + "'," +"'" + data.data.data[i].profit + "'," + "'" + data.data.data[i].scale + "'" + ')">编辑</button>'+
    ...
    function sureAdd(){
        var privateInvestmentId =  getParam("privateInvestmentId");
        var name = $("#name").val();
        var investmentDirection = $("#investmentDirection").val();
        ...
        $.ajax({
            ...
            data : {"privateInvestmentId":privateInvestmentId,
                    "name":name,"investmentDirection":investmentDirection,"profit":profit,"scale":scale},
            dataType : "json",
            success : function (data){
                if(data.success == false){
                    alert(data.errMsg);
    ...
    var updateId;
    function updateById(id,name,investmentDirection,profit,scale){
        $("#name1").val('');
        $("#investmentDirection1").val('');
        $("#profit1").val('');
        $("#scale1").val('');

        $("#name1").val(name);
        $("#investmentDirection1").val(investmentDirection);
        $("#profit1").val(profit);
        $("#scale1").val(scale);
    ...
    function sureUpdate(){
        var name = $("#name1").val();
        var investmentDirection = $("#investmentDirection1").val();
        var profit = $("#profit1").val();
        var scale = $("#scale1").val();
