        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">新增风险控制</h4>
                </div>
                <div class="modal-body" style="margin-left:10px;">
                    <p>级别：<input id="level"/></p>
                    <p>简介：<input id="synopsis"/></p>
                </div>
    ...
    <div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-top: 300px;">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title" id="">修改风险控制</h4>
                </div>
                <div class="modal-body" style="margin-left:10px;">
                    <p>级别：<input id="level1"/></p>
                    <p>简介：<input id="synopsis1"/></p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
...
<script>
            ...
            success : function (data){
                if(data.success == false){
                    ...
                    for(var i=0;i<data.data.data.length;i++){
                        html += '<tr>'+
                                    '<td>'+ data.data.data[i].level +'</td>'+
                                    '<td>'+ data.data.data[i].synopsis +'</td>'+
                                    '<td>'+
                                        '<button type="button" data-toggle="modal" data-target="#updateModal" class="btn btn-info" onclick="updateById(' + "'" + data.data.data[i].id + "',"+ "'" + data.data.data[i].level + "'," + "'" + data.data.data[i].synopsis + "'" + ')">编辑</button>'+
    ...
    var updateId;
    function updateById(id,level,synopsis){
        $("#level1").val('');
        $("#synopsis1").val('');


        $("#level1").val(level);
        $("#synopsis1").val(synopsis);
