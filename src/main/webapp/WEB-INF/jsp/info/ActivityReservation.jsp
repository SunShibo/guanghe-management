...
<script>
  ...
  function getdata(cur,size){
    var title = $("#title").val();
    var type = $("#type").val();
    $.getJSON("/ActivityReservation/list?pageNo="+cur+"&pageSize="+size+"&phone="+title+"&status="+type, function (rs) {
        ...
        "callBack":function(result){
          ...
          for (var i = 0; i < rs.data.data.length; i++) {
            ...
            html += '<tr>'+
                    ...
                    '<td>'+ rs.data.data[i].companyPosition +'</td>'+
                    '<td>'+ rs.data.data[i].corporateName +'</td>'+
                    '<td>'+ homeState +'</td>'
  ...
  function updateState(id){
    $.ajax({
      type : "post",
      url : "/ActivityReservation/update",
      data : {"id":id},
      dataType : "json",
      async : false,
      success : function (data){
        if(data.success == false){
          ...
          alert("修改成功！");
          window.location.href="/ActivityReservation/page";
        }
      }
    });
  }
